...
c_sources = ["pyctlsh/backends/cython/_ctlsh.pyx"] + glob.glob("./ctlsh/*.c")
c_sources = list(filter(lambda x: "main" not in x, c_sources))
extensions = [
    Extension(
        "pyctlsh.backends.cython._ctlsh",
        c_sources,
        include_dirs=["./ctlsh"],
    ),
]
cffi_modules = ["pyctlsh/backends/cffi/build.py:ffibuilder"]
...
def get_version() -> str:
    path = os.path.join(
        os.path.abspath(os.path.dirname(__file__)), "pyctlsh", "__init__.py"
    )
...
def main():
    ...
    setup(
        name="pyctlsh",
        version=version,
        url="https://github.com/synodriver/pyctlsh",
        packages=packages,
        keywords=["ctlsh"],
        description="python binding for ctlsh",
